...
type Dashboard struct {
	...
	gradesKbdTable  *KeyboardScrollTable
...
func (d *Dashboard) buildHeader() *fyne.Container {
	...
	if apiClient.UserInfo != nil {
		userText = apiClient.UserInfo.FullName()
	}
	...
	versionTag := canvas.NewText("v5.5", colorAccent)
...
func (d *Dashboard) loadJournalOptions() {
	...
	for i, g := range opts.Groups {
		classNames[i] = fmt.Sprintf("%d %s", g.ID, g.Name)
	}
...
func (d *Dashboard) onClassSelected(selected string) {
	...
	for i, g := range d.journalOpts.Groups {
		if fmt.Sprintf("%d %s", g.ID, g.Name) == selected {
...
func (d *Dashboard) loadData() {
	...
	apiClient := d.controller.GetClient()
	gID := fmt.Sprintf("%d", d.selectedGroup.ID)
	sID := fmt.Sprintf("%d", d.selectedSubject.SubjectID)
	qID := fmt.Sprintf("%d", d.selectedQuarter.ID)
...
// ------------------------------------------
// GRADES TABLE — selection, arrows, Delete
// ------------------------------------------
...
func (d *Dashboard) rebuildGradesTable() {
	...
	d.gradesTable = widget.NewTable(
		...
		func(id widget.TableCellID, cell fyne.CanvasObject) {
			...
			if id.Row == 0 {
				...
				default:
					...
					if dateIdx >= 0 && dateIdx < len(d.dates) {
						day := d.dates[dateIdx]
						lbl.SetText(day.AssignmentDate[5:])
					}
				}
				return
			}
			...
			case 1:
				...
				lbl.Alignment = fyne.TextAlignLeading
			case totalCols - 1:
				avg := d.calcStudentAverage(student)
				if avg > 0 {
	...
	d.gradesTable.OnSelected = func(id widget.TableCellID) {
		...
		// Re-focus the keyboard-aware wrapper so key events work
		if d.gradesKbdTable != nil {
			w := d.controller.GetWindow()
			if w != nil && w.Canvas() != nil {
				w.Canvas().Focus(d.gradesKbdTable)
			}
		}
	...
	// Wrap table in keyboard-aware scroll container
	d.gradesKbdTable = NewKeyboardScrollTable(d.gradesTable)
	d.gradesKbdTable.onKeyDown = func(ev *fyne.KeyEvent) {
		d.handleJournalKey(ev)
	}
	d.gradesKbdTable.onRune = func(r rune) {
		d.handleJournalRune(r)
	}

	d.gradesContainer.Objects = []fyne.CanvasObject{d.gradesKbdTable}
	d.gradesContainer.Refresh()

	// Install keyboard handler on canvas as fallback
	d.installKeyboardHandler()

	// Auto-focus the keyboard table so it receives key events immediately
	w := d.controller.GetWindow()
	if w != nil && w.Canvas() != nil {
		w.Canvas().Focus(d.gradesKbdTable)
	}
...
// calcStudentAverage computes the average score for a student.
func (d *Dashboard) calcStudentAverage(student client.Student) float64 {
	var sum int
	var count int
	for _, sm := range student.SubjectMarks {
...
func (d *Dashboard) installKeyboardHandler() {
	...
	// Canvas-level fallback — only fires when no Focusable widget has focus
...
// handleJournalKey processes a key event for the journal table.
func (d *Dashboard) handleJournalKey(ev *fyne.KeyEvent) {
	switch ev.Name {
	case fyne.KeyUp:
		d.navigateCell(0, -1)
	case fyne.KeyDown:
...
// handleJournalRune processes a rune for the journal table.
func (d *Dashboard) handleJournalRune(r rune) {
...
func (d *Dashboard) showRandomFillForStudent(studentIdx int) {
	...
	comboNames := make([]string, len(GradeCombos))
	...
	comboSel := widget.NewSelect(comboNames, func(sel string) {
		for _, c := range GradeCombos {
			if c.Name == sel {
				minEntry.SetText(strconv.Itoa(c.Min))
				maxEntry.SetText(strconv.Itoa(c.Max))
				break
			}
		}
	})
	...
	header := fmt.Sprintf("%s %s — %s", student.LastName, student.FirstName, quarterName)
