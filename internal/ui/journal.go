...
func (p *JournalPage) showRandomFillDialog() {
        ...
        fillBtn := widget.NewButton("Заполнить", func() {
                // Save the limits from the dialog entries first
                for name, le := range studentEntries {
                        minV := parseInt(le.minEntry.Text)
                        maxV := parseInt(le.maxEntry.Text)
                        if minV < 1 {
        ...
        descLabel := widget.NewLabelWithStyle(
                "Рандомное заполнение пустых ячеек.\nУстановите мин/макс для каждого ученика.",
                fyne.TextAlignCenter, fyne.TextStyle{Italic: true})

        bottomRow := container.NewHBox(fillBtn, closeBtn)
