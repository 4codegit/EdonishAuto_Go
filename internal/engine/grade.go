        ...
        for _, group := range groups {
                ...
                for _, subject := range subjects {
                        ...
                        for _, quarter := range quarters {
                                ...
                                if len(days) == 0 {
                                        e.log("  ⏭️ Нет дат для этой комбинации", "info")
                                        continue
                                }
...
func (e *Engine) ExecutePlan(plan *GradePlan, numWorkers int, taskDelay time.Duration) {
        ...
        e.log(fmt.Sprintf("🏁 Завершено! ✅ %d успешно, ❌ %d ошибок, ⏭️ %d пропущено", completed, failed, skipped), "info")
        e.updateProgress(plan)
}
...
func (e *Engine) ExecuteQuarterMarks(plan *GradePlan, taskDelay time.Duration) {
        ...
        completed := int(atomic.LoadInt32(&plan.Completed))
        failed := int(atomic.LoadInt32(&plan.Failed))
        e.log(fmt.Sprintf("🏁 Четвертные оценки: ✅ %d, ❌ %d", completed, failed), "info")
}
...
// Н/А (shortName="1/2", mark_value=0) is shown as "отсутствует" on the auto page.
func ParseGradeDisplay(shortName string, markValue int) string {
        if shortName == "" {
                return ""
        }
        // Check for Н/А: mark_value is 0 or shortName represents a fraction < MIN_GRADE
        if markValue == 0 {
                return "отсутствует"
        }
        // Try to parse as fraction (e.g., "1/2")
        // If numerator < MIN_GRADE, it's effectively Н/А
        var num, den int
